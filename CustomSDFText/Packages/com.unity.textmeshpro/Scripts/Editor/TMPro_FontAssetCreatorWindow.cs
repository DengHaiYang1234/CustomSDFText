...
namespace TMPro.EditorUtilities
{
    public class TMPro_FontAssetCreatorWindow : EditorWindow
    {
            ...
            if (GUILayout.Button("Save") && GUI.enabled)
            {
                ...
                if (m_SelectedFontAsset == null)
                {
                    if (m_LegacyFontAsset != null)
                        filePath = SaveNewFontAssetWithSameName(m_LegacyFontAsset);
                    else
                        filePath = SaveNewFontAsset(m_SourceFontFile);
                }
                else
                {
                    // Save over exiting Font Asset
                    filePath = Path.GetFullPath(AssetDatabase.GetAssetPath(m_SelectedFontAsset)).Replace('\\', '/');
                    SaveFontAtlasTexture(filePath);
        ...
        string SaveNewFontAsset(Object sourceObject)
        {
            string filePath;
        ...
        void SaveFontJson(string filePath, TMP_FontAsset fontAsset)
        {
            if (string.IsNullOrEmpty(filePath))
                return;

            string jsonFileName = filePath.Substring(filePath.LastIndexOf('/') + 1);
            if (jsonFileName.IndexOf('.') != -1)
            {
                jsonFileName = jsonFileName.Substring(0, jsonFileName.LastIndexOf('.')) + ".json";
            }
            jsonFileName += ".json";
            var jsonFilePath = Path.Combine(filePath.Substring(0, filePath.LastIndexOf('/')), jsonFileName);

            TMPro_FontJsonStruct jsonStruct = new TMPro_FontJsonStruct();
            jsonStruct.faceInfo = new FontFaceInfo()
            {
                baseline = fontAsset.faceInfo.baseline,
                strikethroughOffset = fontAsset.faceInfo.strikethroughOffset,
                underlineThickness = fontAsset.faceInfo.underlineThickness,
                underlineOffset = fontAsset.faceInfo.underlineOffset,
                subscriptSize = fontAsset.faceInfo.subscriptSize,
                subscriptOffset = fontAsset.faceInfo.subscriptOffset,
                superscriptSize = fontAsset.faceInfo.superscriptSize,
                superscriptOffset = fontAsset.faceInfo.superscriptOffset,
                descentLine = fontAsset.faceInfo.descentLine,
                tabWidth = fontAsset.faceInfo.tabWidth,
                meanLine = fontAsset.faceInfo.meanLine,
                capLine = fontAsset.faceInfo.capLine,
                ascentLine = fontAsset.faceInfo.ascentLine,
                lineHeight = fontAsset.faceInfo.lineHeight,
                scale = fontAsset.faceInfo.scale,
                pointSize = fontAsset.faceInfo.pointSize,
                styleName = fontAsset.faceInfo.styleName,
                familyName = fontAsset.faceInfo.familyName,
                strikethroughThickness = fontAsset.faceInfo.strikethroughThickness,
                padding = fontAsset.creationSettings.padding,
                atlasWidth = fontAsset.creationSettings.atlasWidth,
                atlasHeight = fontAsset.m_CreationSettings.atlasHeight
            };

            int characterTableCount = fontAsset.characterTable.Count;
            jsonStruct.characterTableCount = characterTableCount;
            if (characterTableCount > 0)
            {
                jsonStruct.characterTable = new CharacterTable[characterTableCount];
                for (int i = 0; i < characterTableCount; i++)
                {
                    var item = new CharacterTable();
                    var source = fontAsset.characterTable[i];
                    item.unicode = source.unicode;
                    item.glyphIndex = source.glyphIndex;
                    item.scale = source.scale;
                    item.glyphRect = new FontGlyphRect()
                    {
                        x = source.glyph.glyphRect.x,
                        y = source.glyph.glyphRect.y,
                        width = source.glyph.glyphRect.width,
                        height = source.glyph.glyphRect.height
                    };
                    item.glyphMetrics = new FontGlyphMetrics()
                    {
                        width = source.glyph.metrics.width,
                        height = source.glyph.metrics.height,
                        xOffset = source.glyph.metrics.horizontalBearingX,
                        yOffset = source.glyph.metrics.horizontalBearingY,
                        xAdvance = source.glyph.metrics.horizontalAdvance
                    };
                    jsonStruct.characterTable[i] = item;
                }
            }


            int glyphAdjustmentTableCount = fontAsset.fontFeatureTable.glyphPairAdjustmentRecords.Count;
            jsonStruct.glyphAdjustmentTableCount = glyphAdjustmentTableCount;
            if (glyphAdjustmentTableCount > 0)
            {
                jsonStruct.glyphAdjustmentTable = new GlyphAdjustmentTable[glyphAdjustmentTableCount];
                for (int i = 0; i < glyphAdjustmentTableCount; i++)
                {
                    var item = new GlyphAdjustmentTable();
                    var source = fontAsset.fontFeatureTable.glyphPairAdjustmentRecords[i];
                    
                    item.firstAdjustmentRecord = new GlyphAdjustmentRecord()
                    {
                        glyphIndex = source.firstAdjustmentRecord.glyphIndex,
                        xOffset = source.firstAdjustmentRecord.glyphValueRecord.xPlacement,
                        yOffset = source.firstAdjustmentRecord.glyphValueRecord.yPlacement,
                        xAdvance = source.firstAdjustmentRecord.glyphValueRecord.xAdvance
                    };

                    item.glyphIndex = item.firstAdjustmentRecord.glyphIndex;

                    item.secondAdjustmentRecord = new GlyphAdjustmentRecord()
                    {
                        glyphIndex = source.secondAdjustmentRecord.glyphIndex,
                        xOffset = source.secondAdjustmentRecord.glyphValueRecord.xPlacement,
                        yOffset = source.secondAdjustmentRecord.glyphValueRecord.yPlacement,
                        xAdvance = source.secondAdjustmentRecord.glyphValueRecord.xAdvance
                    };
                }
            }


            var content = JsonUtility.ToJson(jsonStruct, true);
        ...
        void SaveFontAtlasTexture(string filePath)
        {
            if (string.IsNullOrEmpty(filePath))
                return;

            string texFileName = filePath.Substring(filePath.LastIndexOf('/') + 1);
            texFileName = texFileName.Substring(0, texFileName.LastIndexOf('.')) + ".png";
            var texFilePath = Path.Combine(filePath.Substring(0, filePath.LastIndexOf('/')), texFileName);
            if (m_FontAtlasTexture != null)
            {
                Texture2D tex = new Texture2D(m_FontAtlasTexture.width, m_FontAtlasTexture.height, TextureFormat.RGB24, false);

                Color32[] color = new Color32[m_FontAtlasTexture.width * m_FontAtlasTexture.height];

                for (int y = 0; y < m_FontAtlasTexture.height; y++)
                {
                    for (int x = 0; x < m_FontAtlasTexture.width; x++)
                    {
                        Color orgCol = m_FontAtlasTexture.GetPixel(x, y);
                        //color[y * m_FontAtlasTexture.width + x] = new Color32(orgCol.a, orgCol.a, orgCol.a, 255);

                        tex.SetPixel(x, y, new Color(orgCol.a, orgCol.a, orgCol.a, 1));
                    }
                }
                ...
                File.WriteAllBytes(texFilePath, tex.EncodeToPNG());
                AssetDatabase.Refresh();
        ...
        /// </summary>
        /// <param name="sourceObject"></param>
        string SaveNewFontAssetWithSameName(Object sourceObject)
        {
            string filePath;
        ...
        string Save_Bitmap_FontAsset(string filePath)
        {
            ...
            SaveFontJson(filePath, fontAsset);
        ...
        string Save_SDF_FontAsset(string filePath)
        {
            filePath = filePath.Substring(0, filePath.Length - 6); // Trim file extension from filePath.
